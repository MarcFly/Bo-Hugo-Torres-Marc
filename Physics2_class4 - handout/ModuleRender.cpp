...
// Blit to screen
bool ModuleRender::Blit(SDL_Texture* texture, int x, int y, SDL_Rect* section, float speed, double angle, int pivot_x, int pivot_y, bool flip, float scale)
{
	bool ret = true;
	SDL_Rect rect;
	...
	rect.x += (rect.w - rect.w * scale) * 0.5f;
	rect.y += (rect.h - rect.h * scale) * 0.5f;
	rect.w *= scale;
	rect.h *= scale;
